locals {
  ...
  terraformstate_storageaccount_name = substr(replace(lower(format("%s%s%stfstate%s", local.library_env_verified, local.location_short, local.sap_vnet_verified, local.random_id_verified)), "/[^a-z0-9]/", ""), 0, var.azlimits.stgaccnt)
