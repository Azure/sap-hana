...
// Creates storage account for storing ansible YML files
resource "azurerm_storage_account" "storage_ansible" {
  count                     = local.sa_ansible_exists ? 0 : 1
  name                      = local.sa_ansible_name
  resource_group_name       = local.rg_name
  location                  = local.rg_library_location
  account_replication_type  = local.sa_tfstate_account_replication_type
  account_tier              = local.sa_tfstate_account_tier
