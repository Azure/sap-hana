...
resource "azurerm_subnet" "subnet-sap-app" {
  count                = var.application.enable_deployment ? (var.infrastructure.vnets.sap.subnet_app.is_existing ? 0 : 1) : 0
...
data "azurerm_subnet" "subnet-sap-app" {
  count                = var.application.enable_deployment ? (var.infrastructure.vnets.sap.subnet_app.is_existing ? 1 : 0) : 0
...
resource "azurerm_network_security_group" "nsg-app" {
  count               = var.application.enable_deployment ? (var.infrastructure.vnets.sap.subnet_app.nsg.is_existing ? 0 : 1) : 0
...
data "azurerm_network_security_group" "nsg-app" {
  count               = var.application.enable_deployment ? (var.infrastructure.vnets.sap.subnet_app.nsg.is_existing ? 1 : 0) : 0
...
resource "azurerm_subnet_network_security_group_association" "Associate-nsg-app" {
  count                     = var.application.enable_deployment ? (signum((var.infrastructure.vnets.sap.is_existing ? 0 : 1) + (var.infrastructure.vnets.sap.subnet_app.nsg.is_existing ? 0 : 1))) : 0
