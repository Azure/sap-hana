...
resource "azurerm_lb" "anydb" {
  ...
  name                = format("%s-lb", upper(local.anydb_nameprefix))
  resource_group_name = var.resource-group[0].name
  location            = var.resource-group[0].location

  frontend_ip_configuration {
    name                          = format("%s-lbfeip", upper(local.anydb_nameprefix))
...
resource "azurerm_lb_backend_address_pool" "anydb" {
  ...
  name                = format("%s-lbbepool", upper(local.anydb_nameprefix))
}
...
resource "azurerm_lb_probe" "anydb" {
  ...
  resource_group_name = var.resource-group[0].name
  loadbalancer_id     = azurerm_lb.anydb[count.index].id
  name                = format("%s-lbhp", upper(local.anydb_nameprefix))
...
resource "azurerm_availability_set" "anydb" {
  count                        = local.enable_deployment ? 1 : 0
  name                         = format("%s-avset", upper(local.anydb_nameprefix))
  location                     = var.resource-group[0].location
  resource_group_name          = var.resource-group[0].name
